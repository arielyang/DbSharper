...
<xsl:variable name="getCollectionBy" select="./property[@canGetCollectionBy='true' and @canGetItemBy='false']" />
		...
		private Dictionary&lt;<xsl:value-of select="script:CSharpAlias(./@type)" />, int&gt; <xsl:value-of select="script:GetCamelCase(./@columnName)" />ItemIndexer;</xsl:for-each>
		...
		{
			if (<xsl:value-of select="script:GetCamelCase(./@columnName)" />ItemIndexer == null)
			{
				<xsl:value-of select="script:GetCamelCase(./@columnName)" />ItemIndexer = new Dictionary&lt;<xsl:value-of select="script:CSharpAlias(./@type)" />, int&gt;();

				for (int i = 0, length = this.Count; i &lt; length; i++)
				{
					<xsl:value-of select="script:GetCamelCase(./@columnName)" />ItemIndexer.Add(this[i].<xsl:value-of select="./@columnName" />, i);
				}
			}
			
			if (!<xsl:value-of select="script:GetCamelCase(./@columnName)" />ItemIndexer.ContainsKey(<xsl:value-of select="script:GetCamelCase(./@columnName)" />))
			{
				return null;
			}

			return this[<xsl:value-of select="script:GetCamelCase(./@columnName)" />ItemIndexer[<xsl:value-of select="script:GetCamelCase(./@columnName)" />]];
