...
<xsl:if test="@description=''">		/// Summary of <xsl:value-of select="@name" />.</xsl:if>
		...
		{
			<xsl:if test="$resultsCount&gt;0">return </xsl:if><xsl:value-of select="@name" />(model<xsl:for-each select="$inputSqlParameters">
			<xsl:variable name="parameterName" select="@name" />
			<xsl:if test="not(boolean($model/property[@columnName=$parameterName]))">, <xsl:value-of select="@camelCaseName" /></xsl:if>
...
<xsl:if test="@description=''">		/// Summary of <xsl:value-of select="@name" />.</xsl:if>
		...
		</xsl:for-each>, string connectionString)
		{
			...
			global::DbSharper.Library.Data.Database _db = global::DbSharper.Library.Data.DatabaseFactory.CreateDatabase&lt;<xsl:value-of select="/mapping/@databaseType" />&gt;(ConnectionStrings.<xsl:value-of select="/mapping/@connectionStringName" />);
			
			System.Data.Common.DbCommand _dbCommand = _db.<xsl:choose>
			...
			<xsl:variable name="parameterName" select="@name" />
			<xsl:choose>
			<xsl:when test="@direction='Input'">_db.AddInParameter(_dbCommand, "<xsl:value-of select="@sqlName" />", System.Data.DbType.<xsl:value-of select="@dbType" />, <xsl:choose><xsl:when test="boolean($model/property[@columnName=$parameterName])">model.<xsl:value-of select="@name" /></xsl:when><xsl:otherwise><xsl:value-of select="@camelCaseName" /></xsl:otherwise></xsl:choose>);
			</xsl:when>
			<xsl:when test="@direction='InputOutput' or @direction='Output'">_db.AddOutParameter(_dbCommand, "<xsl:value-of select="@sqlName" />", System.Data.DbType.<xsl:value-of select="@dbType" />, <xsl:value-of select="@size" />);
			</xsl:when>
			<xsl:otherwise>_db.AddParameter(_dbCommand, "<xsl:value-of select="@sqlName" />", System.Data.DbType.<xsl:value-of select="@dbType" />, <xsl:value-of select="@size" />, System.Data.ParameterDirection.<xsl:value-of select="@direction" />);
			</xsl:otherwise>
			</xsl:choose>
			</xsl:for-each>
			...
			<xsl:when test="@name='ReturnResult'">(ReturnResult)global::System.Convert.ToInt32(_db.GetParameterValue(_dbCommand, "<xsl:value-of select="@sqlName" />"));</xsl:when>
			<xsl:when test="@type='Guid'">new Guid(_db.GetParameterValue(_dbCommand, "<xsl:value-of select="@sqlName" />").ToString());</xsl:when>
			<xsl:when test="@type='Single'">global::System.Convert.ToFloat(_db.GetParameterValue(_dbCommand, "<xsl:value-of select="@sqlName" />"));</xsl:when>
			<xsl:otherwise>global::System.Convert.To<xsl:value-of select="@type" />(_db.GetParameterValue(_dbCommand, "<xsl:value-of select="@sqlName" />"));</xsl:otherwise>
			</xsl:choose>
			</xsl:if>
			</xsl:for-each>
